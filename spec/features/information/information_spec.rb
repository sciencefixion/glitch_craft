...
RSpec.describe 'Information page' do
  describe 'As a visitor' do
    it "can display information about glitch art" do
      ...
      expect(page).to have_content("Glitch Craft allows you to select an image to glitch from two different sources based off a keyword. The first image is generated fully from scratch based on an AI's preception of the keyword from DeepAI. The second image is found based off human generated perceptions of a keyword through image tags on Pixabay. After selecting an image, we'll cause a random glitch to the image's code and yeild your new glitched masterpiece!")
    end
  end
end
